...
RUN apt-get update
RUN apt-get install -y cmake llvm-14 clang-14 lldb-14 libc++-14-dev libc++abi-14-dev libunwind-14-dev \
...
RUN cd /work/aflpp && make PERFORMANCE=1 LLVM_CONFIG=llvm-config-14 NO_NYX=1 source-only -j `nproc` && make install
